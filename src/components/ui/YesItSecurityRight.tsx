    ...
    <div className={`flex flex-col gap-6 lg:gap-8 items-start justify-start ${className}`}>
      ...
      <div className="flex flex-col gap-8 sm:gap-12 lg:gap-[60px] items-start justify-start w-full">
        {statsData.map((stat, index) => (
          <div 
            key={index}
            className="flex flex-col sm:flex-row gap-4 sm:gap-6 lg:gap-[23px] items-start sm:items-center justify-start w-full"
          >
            {/* Percentage with Gradient */}
            <div 
              className="bg-clip-text font-['Kumbh_Sans',_sans-serif] font-medium text-4xl sm:text-5xl lg:text-[80px] leading-none shrink-0"
              style={{ 
